...
## Introducing the Micro Journal Rev 6
...
# Using the Micro Journal Rev 6 - A Walkthrough

## Setting Up the Micro Journal Rev 6

For setting up your Rev 6, I really can't do much better than Un Kyu lee has already done with his quick start guide,  here:

https://github.com/unkyulee/micro-journal/blob/main/micro-journal-rev-6-one-piece/quickstart.md

Before we dive in, **make sure you have the latest stable firmware for the Rev 6** before you proceed any further. If a version is marked [DEV], that is a version in development. Avoid installing that for now. You can download the current firmware from here:

https://github.com/unkyulee/micro-journal/releases

Then, just remove the SD card from the Rev 6 and use a card reader with your computer to transfer the firmware file to your SD card. Put the SD card back in the Rev 6. When you turn on the Rev 6, you will be asked to acknowledge loading the new firmware.  Then, after some screen flashing, you will be set to go.

## Using the Rev 6 Part 1: The Editor
...
## Using the Rev 6 Part 2: The Menu Screen
...
<u>*Note*</u> The Rev 6 can only connect to 2.4 Ghz Wifi. It won't connect to 5Ghz wifi. Sometimes, especially if you use a router that offers both in a mode where the router decides which is appropriate for the device trying to sync, you may have to go through the setup mmore than once until the router gets it right. After that, you won't have any more trouble.

**BLE KEYBOARD** - This has two functions. "BLE" stands for Bluetooth Low Energy. It makes a limited range connection to either a computer, tablet or phone to allow the Rev 6 to be used as a keyboard for any of those 3 devices. The first time with any device you will have to do the pairing, but after that the connection should be automatic once you invoke it on the Rev 6 by typing "T".
...
## Using the Rev 6 Part 3: Using and Modifying the Rev 6's Key Map.
...
```json
{
    ...
    [
     "ESC", "q", "w", "e", "r", "t", "y", "u", "i", "o", "p", "\b",
     "\t", "a", "s", "d", "f", "g", "h", "j", "k", "l", ";", "\"",
     "LEFT_SHIFT", "z", "x", "c", "v", "b", "n", "m", ",", ".", "UP_ARROW", "RETURN",
     "LEFT_CTRL", "LEFT_GUI", "LEFT_ALT", "LOWER", "", " ", "", "RAISE", "/", "LEFT_ARROW", "DOWN_ARROW", "RIGHT_ARROW"
    ],
    ...
    [
     ...
     "`", "a", "s", "d", "f", "g", "h", "j", "-", "=", "[", "]",
     "LEFT_SHIFT", "z", "x", "c", "v", "b", "n", "m", ",", ".", "/", "RIGHT_SHIFT",
     "LEFT_CTRL", "\\", "LEFT_ALT", "LOWER", "", " ", "", "RAISE", "HOME", "PAGE_DOWN", "PAGE_UP", "END"
    ],
...
Similar basic changes here but adapted for acting as a computer keyboard.  I gave myself the D-Pad navigation and space bar and I moved Enter up. By the way, I found my computer was having trouble with "\n" for Enter, so I used the keyword "RETURN", which works fine.

The biggest change I made was on the raise layer.  I moved F-Keys down a row (which gave me 12 instead of 10) and made the top row the number symbols instead. Why? Same reason as before. I could get the symbols in the default Key Map by holding "LOWER" and Shift, in fact I still can. But I don't want to. I want to just hold "LOWER" for numbers and "RAISE" for symbols.

Hopefully this example helps if you want to make your own changes to either or both Key Maps. You don't have to, but it is nice flexibility if you want to.  Have fun. Make sure, when making changes, you pay attention to the syntax of the data structure - the commas, quotes, that numbers identifying a key don't have quotes, etc. And, ALWAYS, make a backup of your currently working json file before making and testing changes!

Naming convention for each key is based on this source:
