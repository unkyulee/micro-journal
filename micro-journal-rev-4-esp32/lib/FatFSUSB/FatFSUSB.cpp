...
bool FatFSUSBClass::isConnected() const
{
...
{
    Serial.println("FatFSUSBClass::begin()");
...
{
    Serial.println("FatFSUSBClass::end()");
...
extern "C" bool tud_msc_start_stop_cb(uint8_t lun, uint8_t power_condition, bool start, bool load_eject)
{
    if (!start && load_eject)
    {
        // Host is ejecting the device
        FatFSUSB.unplug();
    }
    else if (start && load_eject)
    {
        // Host is re-attaching or powering on
        FatFSUSB.plug();
    }
